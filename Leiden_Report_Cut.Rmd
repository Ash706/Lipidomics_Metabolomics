...
output.directory <- "Z:/_Data/QQQ1/0056_RHAPSODY_Leiden_DCS/Processed/R_Processed/"
...
accepted.calibration.Rsq <- 0.90 #changed from 0.95
...
# data$"ID" <-replace(data$ID, data$ID== "Leu-d10 & Ile-d10; 310.1/140", "Leu-d10; 310.1/140")
# 
# data$"ID"<-replace(data$ID, data$ID=="Leu & Ile; 300.2/130.2" ,"Leu; 300.2/130.2")
...
if ( has.calibration.curves ) {
  ...
  IDs.unique_2<-IDs.unique[grep("Q", IDs.unique, invert = TRUE)]
  for ( i in 1:length( IDs.unique_2 ) ) {
    
    data.plot[ which( data.plot$"ID" == IDs.unique_2[ i ] )[ -1 ],
               c( "Linearity range - low concentration",
